...
import org.fsqrt.minekraft.paper.event.listener
import org.fsqrt.minekraft.paper.extensions.closeForViewers
    ...
    init {
        if (resetOnClose || data.onClose != null) {
            listener<InventoryCloseEvent> {
                handler { event, _ ->
                    if (playerInstances[event.player]?.bukkitInventory != event.inventory) return@handler

                    data.onClose?.invoke(
                        GUICloseEvent(
                            event,
                            playerInstances[event.player]!!,
                            event.player as Player
                        )
                    )

                    if (resetOnClose) {
                        deleteInstance(event.player as? Player ?: return@handler)
                    }
                }
